...
# This repository provides branding and GRUB metadata only. It does not ship a real Linux kernel,
# initrd, or squashfs payload. Those must be generated by a real Ubuntu live-build workflow in the
# CI environment, not copied from the runner's /boot directory.
if [[ ! -s "${ISO_ROOT}/casper/vmlinuz" || ! -s "${ISO_ROOT}/casper/initrd" ]]; then
    echo "Missing real casper/vmlinuz or casper/initrd payload. This repository cannot boot until live-build or a trusted artifact stage provides a genuine kernel and initrd in the ISO root."
    exit 1
fi
